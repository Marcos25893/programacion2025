package PrimeraEvaluacion.Clases.tema3.EjercicioClase;
...
public class Ejercicio3 {
    ...
    public static void main(String[] args) {
        ...
        int ciudad[][]=new int[3][3];
        int contaminacion[][]=new int[3][3];
        int media=0;
        int contador=0;

        for (int i=0;i<ciudad.length;i++){
            for (int j=0;j< ciudad.length;j++){
                ciudad[i][j]=numero_aleatorio(0,100);
                media+=ciudad[i][j];
                contador++;
            }
        }
        ...
        for (int i=0;i<ciudad.length;i++) {
            for (int j = 0; j < ciudad.length; j++) {
                if (ciudad[i][j]<10){
                    ...
                    contaminacion[i][j]=2;
                    System.out.println("Hay mas de 80 de contaminacion en las coordenadas " + i + " " + j);
                }
            }
        }

        pintar(contaminacion);
        System.out.println("La media de la contaminacion es " + (media/contador));
    }
